...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		PowerBorder.shadow:Point("TOPLEFT", -3, 26)
		...
		if (C["unitframes"].classiconportrait == true) then
			local classiconportrait = CreateFrame("Frame", self:GetName().."_PortraitBorder", self)
			classiconportrait:CreateShadow("Default")
			if unit == "player" then
				classiconportrait:CreatePanel("", 32, 32, "TOPRIGHT", health, "TOPLEFT", -5,2)
			elseif unit == "target" then
				classiconportrait:CreatePanel("", 32, 32, "TOPLEFT", health, "TOPRIGHT", 5,2)
			end

			local class = classiconportrait:CreateTexture(self:GetName().."_ClassIcon", "ARTWORK")
		...
		if (unit == "player") then
			...
			if T.level ~= MAX_PLAYER_LEVEL then
				...
				Experience.Text:SetFont(C.media.pixelfont, C["datatext"].fontsize)
			...
			if T.level == MAX_PLAYER_LEVEL then
				...
				Reputation.Text:SetFont(C.media.pixelfont, C["datatext"].fontsize)
			...
			if C["unitframes"].classbar then
				if T.myclass == "DRUID" then
					...
					eclipseBar:Point("BOTTOMLEFT", self, "TOPLEFT", 9, -1)
				...
				if (T.myclass == "WARLOCK" or T.myclass == "PALADIN") then
					...
					bars:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 9, -1)
					bars:Width(131)
					bars:Height(5)
					...
					for i = 1, 3 do					
						...
						if i == 1 then
							...
							bars[i]:Width(128/3) -- setting SetWidth here just to fit fit 250 perfectly
				...
				if T.myclass == "DEATHKNIGHT" then
					...
					Runes:Point("BOTTOMLEFT", self, "TOPLEFT", 9,-1)
					Runes:Height(5)
					Runes:SetWidth(228)
					...
					for i = 1, 6 do
						...
						if (i == 1) then
							Runes[i]:Point("BOTTOMLEFT", self, "TOPLEFT", 9, -1)
						else
				...
				if T.myclass == "SHAMAN" then
					...
					for i = 1, 4 do
						...
						if (i == 1) then
						   TotemBar[i]:Point("BOTTOMLEFT", self, "TOPLEFT", 9, -1)
						else
						   TotemBar[i]:Point("TOPLEFT", TotemBar[i-1], "TOPRIGHT", 7, 0)
						end
						TotemBar[i]:SetStatusBarTexture(normTex)
						TotemBar[i]:Height(5)
						if i == 4 then
							TotemBar[i]:SetWidth(207/4)
						else
							TotemBar[i]:SetWidth(207/4)
						end
						...
						TotemBar[i].FrameBackdrop:SetPoint("TOPLEFT", T.Scale(-2), T.Scale(2))
						TotemBar[i].FrameBackdrop:SetPoint("BOTTOMRIGHT", T.Scale(2), T.Scale(-2))
		...
		if (unit == "target") then			
			...
			for i = 1, 5 do
				...
				if i == 1 then
					if C["unitframes"].charportrait == true then
						CPoints[i]:Point("TOPRIGHT", 79, 4)
					else
						CPoints[i]:Point("TOPRIGHT", 18, 4)
					end
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			elseif unit == "target" then
				...
				castbar:SetHeight(20)
				castbar:Point("TOPRIGHT", self, "BOTTOMRIGHT", 0, 34)
			end
	...
	if (unit == "targettarget") then
		...
		-- Unit name
		local Name = health:CreateFontString(nil, "OVERLAY")
		Name:SetPoint("CENTER", self.Health, "CENTER", 0, 2)
		Name:SetFont(font, 10, "THINOUTLINE")
...
player:SetPoint("BOTTOMLEFT", InvTukuiActionBarBackground, "TOPLEFT", 5,80+adjustXY)
player:Size(246, 84)

-- focus
local focus = oUF:Spawn('focus', "TukuiFocus")
focus:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", -2 - adjust, 350)
focus:Size(180, 28)

-- target
local target = oUF:Spawn('target', "TukuiTarget")
target:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", -5,80+adjustXY)
target:Size(246, 84)

-- tot
local tot = oUF:Spawn('targettarget', "TukuiTargetTarget")
tot:SetPoint("BOTTOM", InvTukuiActionBarBackground, "TOP", 0,78)
tot:Size(130, 82)

-- pet
local pet = oUF:Spawn('pet', "TukuiPet")
pet:SetPoint("BOTTOMRIGHT", TukuiPlayer, "TOPLEFT", -7,7)
if C["unitframes"].extendedpet == true then
	pet:Size(100, 28)
else
	pet:Size(54, 16)
...
if C.arena.unitframes then
	...
	for i = 1, 5 do
		...
		if i == 1 then
			arena[i]:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", 450 + adjust, 180)
		else
...
if C["unitframes"].showboss then
	...
	for i = 1, MAX_BOSS_FRAMES do
		...
		if i == 1 then
			boss[i]:SetPoint("BOTTOMRIGHT", InvTukuiActionBarBackground, "TOPRIGHT", 450 + adjust,180)
		else
