...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
	...
	if not self.text then
		...
		if position:match("TOP") then
			self.text:Point("CENTER", GameTooltipStatusBar, 2, 1)
		else
			self.text:Point("CENTER", GameTooltipStatusBar, 2, 1)
		end
...
local function AddTargetedBy()
	...
	if (numParty > 0 or numRaid > 0) then
		...
		if (#targetedList > 0) then
			...
			line:SetFormattedText("Выбран целью".." (|cffffffff%d|r): %s",(#targetedList + 1) / 3,table.concat(targetedList));
			wipe(targetedList);
		end
	end
end
...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
	...
	if(UnitIsPlayer(unit)) then
		...
		local offset = 2
		if guild then
			_G["GameTooltipTextLeft2"]:SetFormattedText("%s", IsInGuild() and GetGuildInfo("player") == guild and "<|cff0090ff"..guild.."|r> [|cff0090ff"..guildrank.."|r]" or "<|cff00ff10"..guild.."|r> [|cff00ff10"..guildrank.."|r]")
			offset = offset + 1
		end
...
local BorderColor = function(self)
	...
	elseif reaction then
		...
		if quality and quality >= 2 then
			...
			self:SetBackdropBorderColor(unpack(C["media"].bordercolor))
			healthBarBG:SetBackdropBorderColor(unpack(C["media"].bordercolor))
			healthBar:SetStatusBarColor(.150, .150, .150)
		end
	end
